...
- 命令执行通道已结束但没有返回完整 stdout、stderr 和退出码时，每秒使用一次不联网的文件读取操作读取该回执（同一回执），最多等待 60 秒；不得重新执行生图命令。
...
- 回执不存在、无法解析或校验失败时继续等待；校验成功即停止等待并交付结果。最多等待 60 秒后仍未取得有效回执时，只报告“命令结果通道和结果回执均不可用，执行状态未知。”；不得扫描输出目录、按文件数量或时间戳推断结果。
